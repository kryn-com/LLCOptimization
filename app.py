...
if st.button("Calculate Optimization", type="primary"):
    ...
    if box_1 == 0 and box_5 == 0:
        ...
        # --- PREPARE MATH TABLE VARIABLES FIRST ---
        ...
        ui_table = (
            ...
            f'<tr><td>External Funding (Not on 1098-T)</td><td>${ext_funding:,.0f}</td><td>${ext_funding:,.0f}</td></tr>'
            f'<tr style="font-weight: bold; background-color: rgba(128, 128, 128, 0.05);"><td>TOTAL TAXABLE GRANTS/FUNDING</td><td>${baseline["inclusion"] + ext_funding:,.0f}</td><td>${optimized["inclusion"] + ext_funding:,.0f}</td></tr>'
        ...
        if savings > 5:
            ...
            html_report = (
                ...
                f'<tr><td>External Funding (Not on 1098-T)</td><td>${ext_funding:,.0f}</td><td>${ext_funding:,.0f}</td></tr>\n'
                f'<tr style="font-weight: bold; background-color:#fefefe"><td>TOTAL TAXABLE GRANTS/FUNDING</td><td>${baseline["inclusion"] + ext_funding:,.0f}</td><td>${optimized["inclusion"] + ext_funding:,.0f}</td></tr>\n'
                f'<tr><td>Federal AGI</td><td>${baseline["agi"]:,.0f}</td><td>${optimized["agi"]:,.0f}</td></tr>\n'
                f'<tr><td>Federal Tax</td><td>${baseline["fed_tax"]:,.0f}</td><td>${optimized["fed_tax"]:,.0f}</td></tr>\n'
                f'<tr><td>Lifetime Learning Credit</td><td>{base_credit_str}</td><td>{opt_credit_str}</td></tr>\n'
